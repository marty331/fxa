...
describe('lib/metrics', () => {
  ...
  describe('trigger flow.initialize event', () => {
    ...
    it('flow events are triggered correctly', () => {
      ...
      assert.equal(metrics._send.args[0][0].events[0].type, 'flow.signin.foo');
      assert.equal(metrics._send.args[1][0].events[0].type, 'flow.signin.foo');
      assert.equal(metrics._send.args[2][0].events[0].type, 'flow.signin.bar');
      assert.equal(metrics._send.args[3][0].events[0].type, 'flow.baz');
    });
    ...
    it('flow events are triggered correctly with once=true', () => {
      ...
      assert.equal(metrics._send.args[0][0].events[0].type, 'flow.signin.foo');
      assert.equal(metrics._send.args[1][0].events[0].type, 'flow.signup.foo');
    });
  ...
  describe('logEvent', () => {
    ...
    it('flushes events as soon as a flow.* event is logged', () => {
      sinon.stub(metrics, 'flush').resolves({});

      metrics.logEvent('event1');
      metrics.logEvent('event2');
      metrics.logEvent('event3');
      metrics.logEvent('flow.signin');
  ...
  describe('all together now', () => {
    it('flushes as expected', done => {
      ...
      setTimeout(() => {
        try {
          assert.equal(metrics._send.callCount, 5);
          ...
          // subscription is not yet set up
          assert.isUndefined(firstPayload.planId);
          assert.isUndefined(firstPayload.productId);

          assert.equal(metrics._send.args[1][0].events[0].type, 'baz');
          assert.equal(metrics._send.args[1][0].events[1].type, 'loaded');
          assert.equal(metrics._send.args[1][0].events[2].type, 'flow.buz');
          // subscription is now set up
          assert.equal(metrics._send.args[1][0].planId, 'plid');
          assert.equal(metrics._send.args[1][0].productId, 'pid');

          assert.equal(
            metrics._send.args[2][0].events[0].type,
            'signin.complete'
          );
          assert.equal(metrics._send.args[2][0].planId, 'plid');
          assert.equal(metrics._send.args[2][0].productId, 'pid');

          assert.equal(
            metrics._send.args[3][0].events[0].type,
            'signin.password.incorrect'
          );
          assert.equal(
            metrics._send.args[3][0].events[1].type,
            'signin.success'
          );

          assert.equal(
            metrics._send.args[4][0].events[0].type,
            'sent.on.timeout'
          );
          assert.equal(
            metrics._send.args[4][0].events[1].type,
            'inactivity.flush'
          );
        } catch (err) {
          return done(err);
